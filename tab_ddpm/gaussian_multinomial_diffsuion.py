...
class GaussianMultinomialDiffusion(torch.nn.Module):
    ...
    def mixed_loss(self, x, out_dict, mlp_type=None):

        #define the loss of mlp on reconstructed model
        ...
        # model_out argument should last time step of diffusion backprocess (fully reconstructed vector)
        model_recon = self._denoise_fn(
            x_in,
            torch.tensor([0]*x_in.shape[0], device=device).long(),
            **out_dict
        )

        mlp_out = self.mlp_model(model_recon)
        if mlp_type in ['binary_classification', 'regression']:
            y = out_dict['y'].to(torch.float).unsqueeze(dim=-1)
        else:
            y = out_dict['y'].unsqueeze(dim=-1)
        loss_mlp = self.mlp_loss(mlp_out, y)
        ...
        return loss_multi.mean(), loss_gauss.mean(), loss_mlp.mean()
