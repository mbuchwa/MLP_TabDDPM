...
def num_process_nans(dataset: Dataset, policy: Optional[NumNanPolicy]) -> Dataset:
    ...
    if not any(x.any() for x in nan_masks.values()):  # type: ignore[code]
        assert policy is None or policy == '__none__'
        return dataset
...
def cat_drop_rare(X: ArrayDict, min_frequency: float) -> ArrayDict:
    assert 0.0 < float(min_frequency) < 1.0
...
def transform_dataset(
    ...
    if dataset.X_cat is None:
        assert transformations.cat_nan_policy is None or transformations.cat_nan_policy == '__none__'
        assert transformations.cat_min_frequency is None or transformations.cat_min_frequency == '__none__'
        X_cat = None
    else:
        X_cat = cat_process_nans(dataset.X_cat, transformations.cat_nan_policy)
        if transformations.cat_min_frequency is not None or transformations.cat_min_frequency == '__none__':
...
class FastTensorDataLoader:
    ...
    """
    def __init__(self, *tensors, batch_size=32, shuffle=False, device='cuda:0'):
        """
        ...
        self.tensors = [tensor.to(device) for tensor in tensors]
